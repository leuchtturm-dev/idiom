defmodule Idiom.Cache.InsertKeysTest do
  ...
  setup_all do
    initial_state = "test/data.json" |> File.read!() |> Jason.decode!()
  ...
  test "flattens data with colon separating locale, namespace and key" do
    ...
    assert %{{"en", "default", "foo"} => "bar", {"de", "default", "bar"} => "baz"} =
             cache_state
  end
  ...
  test "flattens nested keys with dot separator" do
    data = %{
      "en" => %{
        "default" => %{"foo" => %{"bar" => %{"baz" => "what even comes after baz"}}}
      }
    }
    ...
    assert %{{"en", "default", "foo.bar.baz"} => "what even comes after baz"} =
             cache_state
  end
